...
import { Heart, Check, PlusCircle } from 'lucide-react';

interface HobbiesCardProps {
  form: UseFormReturn<FullRegisterInput, any, any>;
}

export function HobbiesCard({ form }: HobbiesCardProps) {
  ...
  const toggleHobby = (hobby: string) => {
    if (selectedHobbies.includes(hobby)) {
      ...
      setValue('hobbies', [...selectedHobbies, hobby], { shouldValidate: true });
    }
  };
    ...
    <div className="space-y-6">
      <div className="border-b border-neutral-200 dark:border-neutral-800 pb-4">
        <h3 className="text-lg sm:text-xl font-bold text-neutral-900 dark:text-white flex items-center gap-2">
          ...
          <span>الهوايات والشغف الشخصي</span>
        </h3>
        <p className="text-xs sm:text-sm text-neutral-500 mt-1">
          هواياتك تعبر عن شخصيتك وتساعدنا في توجيهك إلى المجموعات والفرق التي تشاركك نفس الشغف.
        </p>
      </div>
      ...
      {errors.hobbies && (
        <p className="text-xs text-red-500 font-medium">{errors.hobbies.message}</p>
      )}

      {/* حقل إضافي لهوايات أخرى */}
      <div className="pt-4 space-y-2 border-t border-neutral-100 dark:border-neutral-800">
        <label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 flex items-center gap-1.5">
          ...
          <span>هل تمارس هوايات أخرى غير مذكورة؟ (اختياري)</span>
        </label>
        <input
          type="text"
          placeholder="اكتب هواياتك أو أنشطتك المفضلة هنا..."
        ...
        />
        {errors.otherHobbies && (
          <p className="text-xs text-red-500 font-medium">{errors.otherHobbies.message}</p>
        )}
      </div>
    </div>
  );
